    ...
    <div className="min-h-screen bg-gradient-to-b from-[#021A34] via-[#0C447C] to-sky-200">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-4 py-6 text-white sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          {/* Bulatan teks diganti menjadi Logo IPB resmi */}
        ...
        <Link to="/login" className="px-4 py-2 text-sm font-semibold rounded-lg bg-white text-[#021A34] hover:bg-slate-100 transition-colors">
          Masuk
        </Link>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-4 pb-16 pt-6 text-white sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
          <div className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/login" className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-lg bg-blue-600 hover:bg-blue-700 text-white transition-colors">
        ...
        <section className="mt-14 rounded-3xl bg-white p-6 text-slate-900 shadow-lg sm:p-8">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {categories.map((name) => (
              <div key={name} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                <div className="text-sm font-semibold text-[#021A34]">{name}</div>
                <div className="text-xs text-slate-600 mt-1">Siap mendukung proses administrasi Anda.</div>
              </div>
            ))}
          </div>
        </section>
